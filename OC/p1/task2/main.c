...
#define NUM_OF_LINES_L2 512				// 512 blocks / 1 block/line = 512 lines
...
void access_cache_L2(uint32_t address, uint8_t *data, int mode) {
	...
	// L1 miss + L2 hit
	// copy block to L1
	if (cache_L2.lines[index].valid && cache_L2.lines[index].tag == tag) {
		if (mode == READ_MODE) {
			memcpy(data, &(cache_L2.lines[index].data[offset]), WORD_SIZE);
			time += L2_READ_TIME;
		}
		else {	
			memcpy(&(cache_L2.lines[index].data[offset]), data, WORD_SIZE);
			cache_L2.lines[index].dirty = 1;
			time += L2_WRITE_TIME;
		}
	}
	...
	// fetch block from RAM and copy it to L1
		...
		if (cache_L2.lines[index].dirty) {
			access_main_memory(address, cache_L2.lines[index].data, WRITE_MODE);
		}

		// start replacing the current line
		cache_L2.lines[index].valid = 1;
		cache_L2.lines[index].dirty = 0;
		cache_L2.lines[index].tag = tag;


		access_main_memory(address, temp, READ_MODE);				// get the block the contains the address
		memcpy(&(cache_L2.lines[index].data[0]), temp, BLOCK_SIZE);	// and put it the current line
		...
		if (mode == READ_MODE) {
			...
			memcpy(&(cache_L2.lines[index].data[offset]), data, WORD_SIZE);
			cache_L2.lines[index].dirty = 1;	// set this line as dirty to indicate that something new was written to it and not to RAM
			time += L2_WRITE_TIME;
		}
	}
}
