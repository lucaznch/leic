...
class Target
{
  constructor(x, y, w, h, l, id, selected_options)
  {
    ...
    this.last_letter = removeAccents(l[l.length - 1]).toLowerCase();
  ...
  {
    if ((mouse_x > this.x && mouse_x < this.x + this.width && mouse_y > this.y && mouse_y < this.y + this.height) && this.selected_options[this.last_letter]) {
      return true;
    }
    return false
  }
  ...
  draw() {
    if (this.selected_options["PELO MENOS UM"]) {
      if (this.selected_options[this.last_letter]) {
        // Draw target
        fill(this.getColor());                 
  ...
  unselect() {
    this.selected_options[this.last_letter] = true;
